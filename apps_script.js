...
function doGet(e) {
  ...
  if (action === 'getAllAttendance') return getAllAttendance(e.parameter);
...
function getStudents(params) {
  ...
  if (!sheet || sheet.getLastRow() < 2) return jsonResp({ students: [] });
  const names = sheet.getRange(2, 1, sheet.getLastRow()-1, 1)
    .getValues().flat().filter(n => n !== '');
  return jsonResp({ students: names });
}
...
function getAllAttendance(params) {
  ...
  const sheetName = `${sessionLabel}_${cls}`;
  const sheet = ss.getSheetByName(sheetName);
  if (!sheet) return jsonResp({ dates: {} });

  const lastCol = Math.max(sheet.getLastColumn(), 1);
  const lastRow = Math.max(sheet.getLastRow(), 1);
  if (lastRow < 2) return jsonResp({ dates: {} });
  ...
  headerRow.forEach((h, colIdx) => {
    if (!h || h.includes('사유') || colIdx === 0) return;
    const memoColIdx = headerRow.indexOf(h + ' 사유');
    const records = [];
    allData.forEach(row => {
      const name = row[0];
      if (!name) return;
      const status = row[colIdx] || '';
      const memo = memoColIdx > -1 ? (row[memoColIdx] || '') : '';
      if (status) records.push({ name, status, memo });
    });
    if (records.length) dates[h] = records;
  });

  return jsonResp({ dates });
}
...
function saveAttendance(data) {
  ...
  records.forEach((rec) => {
    ...
    } else if (rec.status === '조퇴') {
      cell.setBackground('#EDE7F6').setFontColor('#311B92').setFontWeight('bold');
    } else {
  ...
  if (sheet.getColumnWidth(memoCol) < 80) sheet.setColumnWidth(memoCol, 80);
